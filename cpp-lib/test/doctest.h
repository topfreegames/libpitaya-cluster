...
fast_binary_assert(assertType::Enum at,
                   const char* file,
                   int line,
                   const char* expr,
...
template<typename L>
DOCTEST_NOINLINE DOCTEST_FAST_ASSERT_RETURN_TYPE
fast_unary_assert(assertType::Enum at,
                  const char* file,
                  int line,
                  const char* expr,
...
// std::ostream passed as a reference noting that there is a use of an undefined type (which there
// isn't)
...
struct DOCTEST_INTERFACE IReporter
{
    ...
    // traversed - check CurrentTestCaseStats::should_reenter (caching a pointer to the input
...
#define DOCTEST_REGISTER_FUNCTION(f, decorators)                                                   \
    DOCTEST_GLOBAL_NO_WARNINGS(DOCTEST_ANONYMOUS(_DOCTEST_ANON_VAR_)) = doctest::detail::regTest(  \
        doctest::detail::TestCase(                                                                 \
            f, __FILE__, __LINE__, doctest_detail_test_suite_ns::getCurrentTestSuite()) *          \
        decorators);                                                                               \
...
#define DOCTEST_SUBCASE(name)                                                                      \
    if (const doctest::detail::Subcase &                                                           \
            DOCTEST_ANONYMOUS(_DOCTEST_ANON_SUBCASE_)                                              \
...
#define DOCTEST_ASSERT_THROWS_AS(expr, assert_type, ...)                                           \
    do {                                                                                           \
        if (!doctest::getContextOptions()->no_throw) {                                             \
            doctest::detail::ResultBuilder _DOCTEST_RB(                                            \
                doctest::assertType::assert_type, __FILE__, __LINE__, #expr, #__VA_ARGS__);        \
            try {                                                                                  \
                expr;                                                                              \
...
const char*
removePathFromFilename(const char* file)
{
...
void
fastAssertThrowIfFlagSet(int flags)
{
...
int
fileOrderComparator(const void* a, const void* b)
{
...
int
suiteOrderComparator(const void* a, const void* b)
{
    auto lhs = *static_cast<TestCase* const*>(a);
    auto rhs = *static_cast<TestCase* const*>(b);
...
int
nameOrderComparator(const void* a, const void* b)
{
    auto lhs = *static_cast<TestCase* const*>(a);
    auto rhs = *static_cast<TestCase* const*>(b);
...
void
writeStringToStream(std::ostream* s, const String& str)
{
    *s << str;
}
...
void
ContextBuilder::stringify(std::ostream* s) const
{
    int curr = 0;
...
void
ContextScope::stringify(std::ostream* s) const
{
...
void
ResultBuilder::unexpectedExceptionOccurred()
{
    m_threw = true;
    ...
    void test_case_end(const CurrentTestCaseStats& st) override
    {
        ...
        if (crashed || (st.failure_flags & TestCaseFailureReason::Exception)) {
            ...
            successOrFailColoredStringToStream(
                false, crashed ? assertType::is_require : assertType::is_check);
        ...
        // means the test case will be re-entered because there are untraversed (but discovered)
        // subcases
...
bool
parseFlagImpl(int argc, const char* const* argv, const char* pattern)
{
...
bool
parseOptionImpl(int argc, const char* const* argv, const char* pattern, String& res)
{
    for (int i = argc - 1; i >= 0; --i) {
        auto temp = std::strstr(argv[i], pattern);
...
bool
parseCommaSepArgs(int argc, const char* const* argv, const char* pattern, std::vector<String>& res)
{
...
bool
parseIntOption(int argc, const char* const* argv, const char* pattern, optionType type, int& res)
{
    String parsedValue;
    ...
    if (type == 0) {
        ...
        // behavior - assumes '0' on unsuccessful parse...
        ...
        // execute the test if it passes all the filtering
        {
            ...
            do {
                ...
                if (p->should_reenter == true) {
                    DOCTEST_ITERATE_THROUGH_REPORTERS(test_case_end, *g_cs);
                    // remove these flags - it is expected that the reporters have handled these
                    // issues
                    p->failure_flags &= ~TestCaseFailureReason::Exception;
                    p->failure_flags &= ~TestCaseFailureReason::AssertFailure;
                }
...
int
registerReporter(const char* name, int priority, IReporter& r)
{
